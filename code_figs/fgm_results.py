...
def reflect_kde_neg(data, bw="scott", kernel="gau", gridsize=200):
    """KDE for *x ≤ 0* by reflecting negative values across 0."""
    arr = np.asarray(data)
    arr = arr[arr <= 0]
    kde = sm.nonparametric.KDEUnivariate(np.concatenate([arr, -arr]))
    kde.fit(kernel=kernel, bw=bw, fft=False, gridsize=gridsize, cut=0)
    mask = kde.support <= 0
    return kde.support[mask], 2 * kde.density[mask]


def kde_original(data, bw="scott", kernel="gau", gridsize=200):
    """Standard 1-D KDE (no reflection)."""
    kde = sm.nonparametric.KDEUnivariate(np.asarray(data))
...
def panel_A(ax, reps, perc=(25, 50, 75, 100)):
    ...
    for rep in reps:
        ...
        for i, p in enumerate(perc):
            comb[i].extend(rep["dfes"][int(p * (m - 1) / 100)])
...
def panel_B(ax, final):
    ...
    for i, (n, dlist) in enumerate(final.items()):
        x, y = reflect_kde_neg(np.concatenate(dlist))
...
if __name__ == "__main__":
    ...
    for n in n_list:
        ...
        for s in range(repeats):
            ...
            z0 = np.random.normal(size=n)
            _, traj = model.relax(z0, max_steps=max_steps)
            final[n].append(model.compute_dfe(traj[-1]))
